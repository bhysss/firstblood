...
# 查看数据
df_train = pd.read_json('ind_train.json', encoding='utf-8')     # 训练集
df_dev = pd.read_json('ind_dev.json', encoding='utf-8')         # 验证集
df_test = pd.read_json('ind_test.json', encoding='utf-8')       # 测试集

# print(df_train)
# print(df_train[0].head())
# print(df_train[1].head())
# print(df_train[2].head())

# 查看一下一共有多少个标签，每个标签的数目是多少
...
vectorizer = TfidfVectorizer(
    ...
    max_features=max_vocab,  # 最大词数目，如果给定了最大词数目，则只会保留出现频率最高的max_features个词
)
...
pred_labels = clf.predict(dev_sequences)
